...
If you find a security issue in any of the workflows, the validation CI, or in how a template handles untrusted input (webhook payloads, transcripts, customer data flowing into Memory or an LLM), report it via:

- Email: **hello@studiomeyer.io** with subject `[security] n8n-templates`
...
3. Why you believe it is a security issue (data leak, credential exposure, prompt-injection vector, etc).
...
- A template whose memory writes do not de-duplicate.
...
1. n8n trigger node (webhook / chat / schedule).
2. (Optional) StudioMeyer Memory operations.
3. LLM provider (OpenAI / Anthropic / etc).
4. (Optional) Memory writes.

Each layer has its own security profile. Memory uses OAuth 2.1 + multi-tenant isolation at [memory.studiomeyer.io](https://memory.studiomeyer.io). LLM providers handle their own auth. n8n itself is the perimeter the builder is responsible for. The templates do not introduce additional trust boundaries.

If you find a security issue in StudioMeyer Memory itself (not a template that uses it), report it at the [n8n-nodes-studiomeyer-memory](https://github.com/studiomeyer-io/n8n-nodes-studiomeyer-memory) repo or to the same email.
