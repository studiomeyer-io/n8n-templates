# Cover Image Spec, Restaurant Stammgast-Bot
...
```
A clean technical illustration on a deep navy background. Three icons in a horizontal row connected by glowing arrows: (1) a Telegram chat-bubble icon on the left in soft blue, (2) a knowledge-graph node cluster in gold center showing a customer entity with branched observations, (3) a chef hat or restaurant fork-and-knife icon on the right in soft cyan. Below the icons in bold sans-serif white text: "Restaurant Stammgast-Bot". Below that in smaller gold text: "n8n + StudioMeyer Memory". Minimalist, high contrast, no clutter, suitable for a developer marketplace listing.
```
