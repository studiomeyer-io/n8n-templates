# Cover Image Spec, Tourist-Bot Repeat-Visitor
...
```
A clean technical illustration on a deep navy background. Three icons in a horizontal row connected by glowing arrows: (1) a globe-and-compass tourism icon on the left in soft blue, (2) a knowledge-graph node cluster in gold center showing a visitor entity with branched session observations, (3) a chat-bubble with a multilingual conversation icon on the right in soft cyan. Below the icons in bold sans-serif white text: "Tourist-Bot Repeat-Visitor". Below that in smaller gold text: "n8n + StudioMeyer Memory". Minimalist, high contrast, no clutter, suitable for a developer marketplace listing.
```
