...
- **Template 01 "async memory writes" claim** softened. Previous wording promised "no latency impact, async after response" which n8n's `executionOrder: v1` does not strictly guarantee. New wording is honest: "execution-order-dependent, walks breadth-first from Normalize LLM Output and hits Respond first because it's wired ahead of the memory writes in the connections array." Mentions Execute-Workflow / queue migration paths for stricter guarantees. Caught by external audit.

### Added (Template 03 v0.4.0 hardening, listed for completeness, see commit d4a83e9 on the same day)
