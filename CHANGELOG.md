...
- **Template 01 Async-Latency Sticky Note finally fixed.** The previous v0.4.0-prep CHANGELOG entry claimed the "no latency impact" wording was softened. It was softened in the Common-Gotchas + Architecture sections of the README but the Sticky Note INSIDE `templates/01-voice-agent-cross-session-memory/workflow.json:417` still said *"they don't add latency to the call"*. ChatGPT's audit caught this. Now wording is honest: "Memory writes (after Respond, execution-order-dependent) ... This is execution-order-dependent, not a hard async guarantee. Verify in your n8n setup if call latency is critical. For a stricter contract use Execute-Workflow trigger or a queue (Redis, BullMQ, n8n Queue Mode)." Plus the sticky note is renamed from "Sticky Note - Async Writes" to "Sticky Note - Memory Writes" so the file structure mirrors the honest framing.
- **executionOrder v1 description corrected (3-agent fact-check).** The README + workflow sticky note had the n8n executionOrder v1 semantics inverted. v1 runs each branch **depth-first** ordered by canvas position (top-to-bottom, then left-to-right), not breadth-first by connections-array order. v0 was breadth-first. The wording was wrong in 4 places (T01 README architecture section, T01 README common-gotchas, T01 workflow.json sticky note, the v0.4.0-prep CHANGELOG entry that introduced the original "softened" claim). All four corrected. Source: [docs.n8n.io/flow-logic/execution-order](https://docs.n8n.io/flow-logic/execution-order/).
- **Vapi `end-of-call-report` example payload shape corrected (3-agent fact-check).** The first version of `examples/vapi-end-of-call.json` placed `transcript`, `messages[]`, and `summary` at the `message.*` root level. The current Vapi schema has them under `message.artifact.{transcript,messages,recordingUrl,stereoRecordingUrl}` and `message.analysis.summary`. Anyone wiring `{{ $json.body.message.transcript }}` against real Vapi traffic would have hit `undefined`. Fixed.
- **Retell `call_ended` example payload corrected (3-agent fact-check).** The first version of `examples/retell-call-ended.json` included `call.call_analysis` under the `call_ended` event. Retell explicitly excludes that field from `call_ended` (the analysis ships in a separate `call_analyzed` event after post-call processing). Removed `call_analysis` and added an explanatory `_comment` line so anyone needing the analysis listens for the right event.
- **Free-tier credit count drift across the repo.** README, FAQ, all three template READMEs (cost notes, tech-stack matrix, comparison table), and PRODUCTION_CHECKLIST claimed "10k ops/month" or "10 000 operations". The Memory product's portal Server-Action `startFreeMemory()` actually grants **200 free credits** (one credit per operation). Eleven occurrences updated to be self-consistent with the n8n-nodes README v0.1.2 hot-fix and the actual portal behavior.
- **CVE-2026-27493 floor wording precision.** The "n8n 2.10.1+" floor named only the latest channel. The CVE patch is in **2.9.3 (stable channel)**, **2.10.1 (latest channel)**, and **1.123.22 (1.x LTS)**. README, FAQ, all three template READMEs, and PRODUCTION_CHECKLIST updated to name all three patched-version-or-newer combinations so 2.x stable users (the n8n.io default) are not steered at the wrong floor. The README badge stays on `2.10.1+` as the simplest single-line ask. Source: [github.com/n8n-io/n8n/security/advisories/GHSA-75g8-rv7v-32f7](https://github.com/n8n-io/n8n/security/advisories/GHSA-75g8-rv7v-32f7).
- **n8n Cloud `crypto` auto-allow claim softened.** T01 README setup callout and PRODUCTION_CHECKLIST said "n8n Cloud allows `crypto` automatically and needs no flag". n8n Cloud's allowlist for individual Node.js builtins is not publicly documented as of 2026-05. Updated wording to "verify in your Cloud instance with a one-line `require('crypto')` test before enabling HMAC in production".
- **README FAQ "Are these production-ready?"** rewritten. The FAQ still framed Template 01 as "developer-preview until next pass" while STATUS.md said T01 was Hardened. Drift removed: all three are at the same code-level hardening, T01 is the only one without an end-to-end live voice smoke yet. STATUS.md and PRODUCTION_CHECKLIST.md cross-linked from the FAQ.
- **README Templates-Tabelle (Tier 1 section)** updated. Section header changed from "Maximum ROI · Live" to "Maximum ROI · Hardened (v0.4.0-prep)" and a Status column added per template. Reads as ground truth, not aspirational.

### Added

- **`PRODUCTION_CHECKLIST.md`** at the repo root. Single page that names every env var (`VAPI_SIGNING_SECRET` / `RETELL_SIGNING_SECRET` / `IDEMPOTENCY_ENABLED` / `RATE_LIMIT_ENABLED` / `WEBHOOK_INTEGRITY_CHECK_ENABLED` / `NODE_FUNCTION_ALLOW_BUILTIN=crypto` for self-host T01 / Telegram `secretToken`), Redis cluster swap snippet for `$getWorkflowStaticData`, reverse-proxy rate-limit pattern (Nginx / Cloudflare / Traefik), monitoring hooks, secrets-management guidance, smoke-test recipe using `examples/`, and a copy-paste pre-launch checklist. The checklist row "If a row is unchecked, the deployment is developer-preview, not production" frames the whole thing.
- **`examples/` folder filled.** Three sample provider payloads:
  - `examples/vapi-end-of-call.json` - Vapi `end-of-call-report` shape
  - `examples/retell-call-ended.json` - Retell `call_ended` shape
  - `examples/telegram-message.json` - Telegram Bot API `Update` shape
  - Plus `examples/README.md` with curl recipes for unsigned payload, signed payload (HMAC-SHA256), idempotency replay, and Telegram secret_token. All three payloads share one fictional customer (Maria Schmidt, +49170555444) so smoke-tests can chain across templates and watch one caller persist across channels.
- **`Why trust the Memory backend` section** in the top-level README. Ten-row trust signals table covering npm provenance, source + changelog, security policy, EU hosting + DSGVO, data export + delete (Art. 17), bi-temporal model, multi-tenant isolation (static-analysis CI guard), API-key rotation, OAuth 2.1 + API key auth modes, open-source posture (templates and node MIT, server hosted-only). Honest about the hosted-only server with a contact path for self-host requirements.
- **`NODE_FUNCTION_ALLOW_BUILTIN=crypto` setup gotcha** prepended to Template 01 README Setup section as a quoted callout. Self-hosted n8n needs the env var for the Verify Webhook Code node's `require('crypto')`. Cross-link to PRODUCTION_CHECKLIST.md and the n8n Docs page.
- **CI Smoke-Test stub** at `.github/workflows/smoke-test-snippets.yml`. Two jobs ship today: payload-schema (validates every example/*.json is valid JSON) and workflow-payload-pairing (T01 needs vapi or retell example, T02+T03 need telegram example). The snippet-execution job is stubbed with a tracker pointer to a future full harness that extracts `parameters.jsCode` from each Code node and runs it against the matching example payload in a sandbox.

### Added

- **Template 01 hardened to v0.4.0 production-pattern standard** (third template after 02 + 03, completing the Tier-1 hardening pass). Three new opt-in Code nodes between Webhook and Normalize Payload (Verify Webhook with VAPI_SIGNING_SECRET / RETELL_SIGNING_SECRET, Rate Limit, Idempotency Check on callId) gated by env vars, default-off so the import still boots clean. The Webhook node has `rawBody: true` enabled for HMAC verification against the original byte stream. Both LLM Reply nodes have `onError: continueErrorOutput` wired to a new `LLM Fallback Reply` Code node which feeds Respond to Voice Provider (graceful voice-friendly TTS message) plus a new `Memory: Learn Error` write with `category: mistake`. The `LLM Fallback Reply` discriminates between LLM-error arrival paths and Route-by-Provider router-fallback (typo or unknown provider) so the audit trail does not leak the user's private memory context. Voice fallback message is intentionally short ("Sorry, I'm having trouble, please call back in a minute") because TTS reads it aloud.
- **Real `Has Phone?` IF in Template 01** addresses the long-standing comment-vs-flow drift flagged by the external audit. The IF after Normalize Payload branches on `hasPhone`: true → full memory path runs, false → directly to Build LLM Prompt with an "anonymous caller" system prompt, skipping Memory entirely on both lookup and write paths. A second `Has Phone? (post)` IF after Normalize LLM Output gates Memory: Observe + Memory: Learn Outcome so anonymous calls never write polluted Memory either. Memory: Learn Error from the LLM fallback fires regardless (LLM errors are valuable even for anonymous callers).
- **Build LLM Prompt anonymous-aware in Template 01** with two-branch system prompt. Phone path includes caller phone + memory context. Anonymous path tells the LLM "treat as first-time, ask for an email or callback number if continuity matters" and skips the memory context lookup entirely.
- **STATUS.md added** at the repo root as single source of truth for what's hardened, what's developer-preview, where distribution stands. Per-template status table, repo-level checklist, "what 'hardened' means" definition (the six-phase pass), explicit "what's NOT yet done" section. Driven by external audit's request for explicit status messaging.

### Fixed

- **Template 01 Anthropic node type-string** corrected from `n8n-nodes-base.anthropic` (does not exist in n8n core) to `@n8n/n8n-nodes-langchain.anthropic` with `resource: text + operation: message` parameters. Same bug-class fixed in Templates 02 + 03 earlier this session. Verified against n8n 2.15.0 pre-activation check (only OpenAI + Anthropic credential-missing remain, no Unrecognized node type).
- **Template 01 internal style guide section order** restored. Multi-Provider Switch was at position 2 (immediately after What this does), now at position 5 (between Setup and Extending) per internal section 7. Tech Stack Matrix and Credentials Checklist were before Common gotchas, now after Hard Compatibility Floor per internal style guide sections 10d/10e.
- **Template 01 HMAC raw-body access** corrected. Earlier fix iteration used `item.body.toString()` which on a parsed JSON object returns the literal string `[object Object]`, making HMAC verification fail for every legitimate webhook when the signing secret was configured. Fixed to read `item.rawBody` (the field n8n's Webhook node populates when `rawBody: true` is enabled) with a clean fallback chain. Caught by 3-agent code review, no production impact (no real Vapi/Retell account has been wired yet, this is the developer-preview ship).
- **Top-level README n8n-compatibility badge** corrected from `n8n-1.50+` to `n8n-2.10.1+` to match the CVE-2026-27493 floor declared in every template README. Bug was a leftover from before the CVE-aware floor was added.
- **GitHub repo description** updated from "Production-ready n8n workflow templates with cross-session memory" to "Production-pattern hardened n8n workflow templates with cross-session memory" so the public-facing claim matches the README's honest framing. Per the new repo-internal rule: "Public repo description never claims more than the weakest production-ready path in the repo claims."
- **Template 01 cost table** corrected from `gpt-5-mini $0.15 / 1M input` to `$0.25 / 1M input + $2.00 / 1M output` (current 2026 pricing). Per-call cost recalculated. Same fix already applied to 02 + 03.
- **Template 01 "async memory writes" claim** softened. Previous wording promised "no latency impact, async after response" which n8n's `executionOrder: v1` does not strictly guarantee. New wording is honest: "execution-order-dependent, runs each branch depth-first ordered by canvas position (top-to-bottom, left-to-right), so `Respond to Voice Provider` executes first because it sits ahead on the canvas." Mentions Execute-Workflow / queue migration paths for stricter guarantees. Caught by external audit. *(Note: an earlier version of this CHANGELOG entry described executionOrder v1 as "breadth-first" - that was a residual error from the old v0 mental model. v1 is depth-first; the actual code in the README + workflow.json was corrected to match in the 2026-05-01 fact-check pass.)*

Template 03 v0.4.0 hardening, listed for completeness, see commit d4a83e9 on the same day.
...
Template 02 v0.4.0 hardening, listed for completeness, see commit 0a3af63 on the same day.
...
- **Hard compatibility floor section** as standalone block in Template 02 README (matching internal section 10c).
...
- **Anthropic model in Template 03** corrected from `claude-sonnet-4-6` to `claude-haiku-4-5` (matches README and internal style guide default).
...
- **internal style guide section order in Template 03 README** restored: Common gotchas before Production Patterns, Tech Stack and Credentials Checklist after Hard Compatibility Floor.
...
- The em-dash guard in `.github/workflows/validate-workflows.yml` already enforced the no-em-dash rule, but the rule is now also called out explicitly in the internal style guide under "Voice-Regeln".
